...
void
GATTRequester::write16_by_handle_async(uint16_t handle, uint16_t data,
                                     GATTResponse* response) {
    check_channel();
    gatt_write_char(_attrib, handle, (const uint8_t*)&data, 2,
                    write_by_handle_cb, (gpointer)response);
}

void
GATTRequester::write16_by_handle(uint16_t handle, uint16_t data) {
    write16_by_handle_async(handle, data, NULL);
}
